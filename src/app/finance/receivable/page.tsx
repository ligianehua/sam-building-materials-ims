...
export default function ReceivablePage() {
  ...
  const [form, setForm] = useState({ customerId: "", amount: 0, dueDate: "", description: "" });

  const fetchData = async () => {
    setLoading(true);
    const params = new URLSearchParams({ type: "RECEIVABLE" });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    await fetch("/api/invoices", {
      ...
      body: JSON.stringify({ ...form, type: "RECEIVABLE", date: new Date().toISOString() }),
    });
    setShowAdd(false);
    setForm({ customerId: "", amount: 0, dueDate: "", description: "" });
    fetchData();
  };
  ...
  return (
    <div>
      <PageHeader title="Accounts Receivable" description="Track customer payments and outstanding invoices" actions={<Button onClick={() => setShowAdd(true)}><Plus size={16} /> New Invoice</Button>} />
      ...
      {loading ? (
        ...
        <EmptyState title="No receivable invoices" description="Create your first invoice." action={<Button onClick={() => setShowAdd(true)}><Plus size={16} /> New Invoice</Button>} />
      ) : (
      ...
      {/* Add Invoice Modal */}
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="New Receivable Invoice">
        <form onSubmit={handleAdd} className="space-y-4">
          <Select label="Customer *" options={[{ value: "", label: "Select customer..." }, ...customers.map((c) => ({ value: c.id, label: c.company }))]} value={form.customerId} onChange={(e) => setForm({ ...form, customerId: e.target.value })} required />
          ...
          {showPayment && (
            <>
              <div className="bg-muted/50 rounded-lg p-3 text-sm space-y-1">
                <p>Invoice Amount: <strong>{formatCurrency(showPayment.amount)}</strong></p>
